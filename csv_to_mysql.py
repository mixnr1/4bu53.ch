...
#create table which has Primary Key
...
mycursor.execute("CREATE UNIQUE INDEX `abuseidx` ON abuse (`dateadded`,`protocol`,`domain`, `url_status`,`threat`,`tags`,`urlhaus_link`,`reporter`);")
        ...
        with open(os.path.join(csv_dir, csv_f)) as csv_file:
            ...
            for row in csv_reader:
                if row[0].startswith('#'):
                    ...
                    reporter = str(row[7])
                    rline = (dateadded, protocol, domain, url_status, threat, tags, urlhaus_link, reporter)
                    val.append(rline)
            ...
            #     ) 
            #     mycursor = mydb.cursor()
            #     sql = "INSERT IGNORE INTO abuse (dateadded, protocol, domain, url_status, threat, tags, urlhaus_link, reporter) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            ...
            for i in val:
                ...
                mycursor = mydb.cursor()
                sql=("SELECT count(abuseid) FROM abuse WHERE dateadded='%s' AND protocol='%s' AND domain='%s' AND url_status='%s' AND threat='%s' AND tags='%s' AND urlhaus_link='%s' AND reporter='%s';")
                mycursor.execute(sql % (dateadded, protocol, domain, url_status, threat, tags, urlhaus_link, reporter))
